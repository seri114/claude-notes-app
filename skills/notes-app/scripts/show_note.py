#!/usr/bin/env python3
"""Show a note from Notes.app as Markdown to STDOUT.
...
def main() -> int:
    ...
    if not title:
        print("Usage: show_note.py \"Title\"", file=sys.stderr)
        return 1
...
tell application "Notes"
    ...
    return body of targetNote as Unicode text
end tell
'''
    ...
    if result.returncode == 0 and not output.startswith("ERROR:"):
        # Debug: dump HTML for inspection
        if '--html' in sys.argv:
            print(output)
            return 0
        # Convert HTML to Markdown and print
        markdown = html_to_markdown(output)
        print(markdown)
        return 0
    else:
